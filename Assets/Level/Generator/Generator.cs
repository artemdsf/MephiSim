...
public class Generator : MonoBehaviour
{
	...
	protected Room AddNewHallway(Room lastRoom, int hallwayMaxLenght, int chanceToChangeRoom = 100)
	{
		...
		for (int hallwayLenght = 1; hallwayLenght < hallwayMaxLenght; hallwayLenght++)
		{
			...
			{
				LevelInfo.Map.Remove(newHallway);
	...
	protected void GenerateFloor()
	{
		...
		foreach (var room in LevelInfo.Map)
		{
			GenerateRoom(room);
		}
	...
	private void CopyRoomPrefab(Room room, RoomPrefab prefab)
	{
		...
		for (int i = 0; i < LevelInfo.ChunkSize.x; i++)
		{
			for (int j = 0; j < LevelInfo.ChunkSize.y; j++)
			{
				tilePositionInPrefab = prefab.LeftDownCorner + new Vector3Int(i, j, 0);
				if (tilemap.HasTile(tilePositionInPrefab))
				{
					tilePositionInRoom = room.Position * LevelInfo.ChunkSize + new Vector3Int(i, j, 0);
		...
		{
			roomPosition = room.Position * LevelInfo.ChunkSize + child.position - prefab.LeftDownCorner;
	...
	{
		int left = LevelInfo.LeftPosition;
		int right = LevelInfo.RightPosition;
		int down = LevelInfo.DownPosition;
		int up = LevelInfo.UpPosition;
	...
	private Room GetExtremeRoom(out Vector3Int directionToSpawn)
	{
		...
		int start = LevelInfo.DownChunk;
		int finish = LevelInfo.UpChunk;
		...
		{
			case 0:
				extremeChunk = LevelInfo.LeftChunk;
				directionToSpawn = Vector3Int.left;

				start = LevelInfo.DownChunk;
				finish = LevelInfo.UpChunk;

				edgeAxis = Vector3Int.right;
				direction = Vector3Int.up;
				break;
			case 1:
				extremeChunk = LevelInfo.UpChunk;
				directionToSpawn = Vector3Int.up;

				start = LevelInfo.LeftChunk;
				finish = LevelInfo.RightChunk;

				edgeAxis = Vector3Int.up;
				direction = Vector3Int.right;
				break;
			case 2:
				extremeChunk = LevelInfo.RightChunk;
				...
				break;
			case 3:
				extremeChunk = LevelInfo.DownChunk;
		...
		{
			room = LevelInfo.GetRoom(edgeAxis * extremeChunk + direction * i);
